...
class SingleplayerPage extends React.Component {
  ...
  _onKeyDown = (event) => {
    ...
    switch(event.keyCode) {
      case 37:
        if(this._checkLeft(playerInfo.path)) {
          playerInfo.direction = 37;
        }
        break;
      case 38:
        if(this._checkForward(playerInfo.path)) {
          playerInfo.direction = 38;
        }
        break;
      case 39:
        if(this._checkRight(playerInfo.path)) {
          playerInfo.direction = 39;
        }
        break;
      case 40:
        if(this._checkBackward(playerInfo.path)) {
          playerInfo.direction = 40;
        }
        break;
      case 68:
        playerInfo.view = 'detect';
        break;
      case 70:
        playerInfo.view = 'front';
        break;
      case 71:
        playerInfo.view = 'gaze';
        break;
      case 83:
        playerInfo.view = '';
        break;
  ...
  move = () => {
    let playerInfo = this.state.player;
    let playerVertices = playerInfo.path;
    let next = new THREE.Vector3();
    const now = playerVertices[playerVertices.length - 1];
    if(this._shouldDown(playerVertices)) {
      next.set(now.x, now.y, now.z - this.sideLength);
    } else if(this._shouldUp(playerVertices, playerInfo.direction)) {
      next.set(now.x, now.y, now.z + this.sideLength);
    } else {
    ...
    playerInfo.path = playerVertices;
    this.setState({ 
      player: playerInfo,
      food: foods,
    });
  ...
  render() {
    ...
    } else if(this.state.player.view === 'front') {
      cameraRotate = this.gridRotate;
      cameraPosition = new THREE.Vector3(0, 0, this.sideLength);
      cameraPosition.add(userPath[userPath.length-1]);
